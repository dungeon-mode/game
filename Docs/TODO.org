...
**** INPROGRESS Turning on Play mode should
     * DONE Turn on predicated-drawing
     * DOME Turn off draw-complete-level
     * TODO Load seen cells for current level, if any
     * TODO Set current position to general store if not otherwise known
