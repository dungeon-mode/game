...
The authoritative upstream is Non-GNU Savannah:
...
There are some demos and video from the [[https://emacsconf.org/2020/talks/][EmacsConf 2020 talks]].
...
Erik is getting frustrated drawing tiles, layer by layer, by hand, so getting dm-sketch flushed out (or maybe replaced?) has become a priority.

In theory, we have some working POC code, that just needs a bit of love to cobble together enough work-flow to get Erik going again.  Ideally, we'd like to yank the sketch program out and make it a stand-alone tile editor.

If it's not clear: we'd love help in this area, anything from reviewing and discussing what we have, to running off in your own direction with it.  Please open up an [[https://savannah.nongnu.org/bugs/?group=dungeon&func=additem][issue on Savannah]] if you're interested!
