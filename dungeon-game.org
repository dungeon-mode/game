...
# #+INCLUDE: "src/classes.el" src elisp :LINES "26-88"
...
#+BEGIN_SRC elisp :padline yes
  ...
  No abstraction of remote buffer **state**, moreover MODE and
  P-MODE establish setting when creating and sometimes updating
  buffers but don't really work at preventing changes post-facto.")
  ...
  (defclass dm-play-session ()
    ((players :initarg :player-list)
     (sources :initarg :game-sources)
     (remote-buffers :initarg :remote-display-settings)
     (refs :initarg :active-remote-source-references)
     (action-token :initarg action-token))
    "Represent a single play session in progress or after the fect.")
